...
async function addQuestion() {
    try {
        ...
        // Check for duplicate
        const existing = await Question.findOne({ text: 'What do you do?', image: 'q_hrt_rmd_4.png' });
        if (existing) {
            ...
            options: [
                {
                    text: 'Show Answer',
                    isCorrect: true,
                    explanation: 'In women with APS (aPL+ and history of clot) who ARE on anticoagulation: AVOID HRT (conditionally recommend against).'
                }
            ],
            summary: 'ACR Reproductive Health Guidelines: For patients with APS who are already on anticoagulation, HRT is conditionally recommended against (avoided).',
            diagram: true,
            difficulty: 'medium',
            image: 'q_hrt_rmd_4.png',
            image2: 'a_hrt_rmd_4.png',
            showImageWithQuestion: true,
            isFreeTrialQuestion: false
        });

        console.log('✅ Question 4 added successfully!');
