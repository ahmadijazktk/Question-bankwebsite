
import mongoose from 'mongoose';
import dotenv from 'dotenv';
import path from 'path';
import fs from 'fs';
import crypto from 'crypto';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
dotenv.config({ path: path.join(__dirname, '.env') });

const imagesDir = path.join(__dirname, '..', 'src', 'images');

const QuestionSchema = new mongoose.Schema({}, { strict: false });
const Question = mongoose.models.Question || mongoose.model('Question', QuestionSchema);

function getHash(filename) {
    if (!filename) return null;
    const p = path.join(imagesDir, filename);
    if (!fs.existsSync(p)) return 'MISSING_' + filename;
    return crypto.createHash('md5').update(fs.readFileSync(p)).digest('hex');
}

async function cleanup() {
    await mongoose.connect(process.env.MONGO_URI);
    const questions = await Question.find({ text: /What do you do\??/i });
    console.log(`Found ${questions.length} 'What do you do?' questions.`);

    const groups = new Map();

    questions.forEach(q => {
        const h = getHash(q.image);
        const expl = (q.options && q.options[0] && q.options[0].explanation || "").trim();
        const category = q.category || "Uncategorized";

        // Logical Key: Image Content + Explanation
        // Special case: if explanation is generic "Review the provided image", we try to match by image only
        const isGeneric = /Review the provided image|Show Answer/i.test(expl);

        let key;
        if (isGeneric) {
            key = `IMG_${h}`;
        } else {
            key = `IMG_${h}_EXPL_${expl.substring(0, 100)}`;
        }

        if (!groups.has(key)) groups.set(key, []);
        groups.get(key).push(q);
    });

    const toDelete = [];

    for (const [key, list] of groups.entries()) {
        if (list.length > 1) {
            console.log(`\nDuplicate Set Found for key: ${key.substring(0, 50)}...`);

            // Heuristic: Keep the one with actual explanation. 
            // If all are the same, keep the most recent one.
            list.sort((a, b) => {
                const explA = (a.options?.[0]?.explanation || "").trim();
                const explB = (b.options?.[0]?.explanation || "").trim();
                const genA = /Review the provided image|Show Answer/i.test(explA);
                const genB = /Review the provided image|Show Answer/i.test(explB);

                if (genA && !genB) return 1;
                if (!genA && genB) return -1;

                return (b.createdAt || 0) - (a.createdAt || 0);
            });

            console.log(`Keeping: ${list[0]._id} (Img: ${list[0].image}, Expl: ${list[0].options?.[0]?.explanation?.substring(0, 30)}...)`);
            for (let i = 1; i < list.length; i++) {
                console.log(`Deleting: ${list[i]._id} (Img: ${list[i].image})`);
                toDelete.push(list[i]._id);
            }
        }
    }

    if (toDelete.length > 0) {
        console.log(`\nProceeding to delete ${toDelete.length} duplicate questions...`);
        const res = await Question.deleteMany({ _id: { $in: toDelete } });
        console.log(`Deleted ${res.deletedCount} items.`);
    } else {
        console.log("\nNo duplicates found to delete in this pass.");
    }

    await mongoose.disconnect();
}
cleanup();
